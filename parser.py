...
def run_code(filename, docker_image):
    try:
        ...
        if result.returncode == 0:
            ...
            return output_filename, f"exit code: {exit_code}"
        else:
            exit_code = "1: fail"
            # Return the error message as a string
            print(f"exit code {result.returncode}: fail")
            error_filename = f"{os.path.splitext(filename)[0]}_error.txt"
            with open(error_filename, 'w') as f:
                f.write(f"Error: {result.stderr}")
            return error_filename, f"exit code: {exit_code}"
        ...
        error_filename = f"{os.path.splitext(filename)[0]}_error.txt"
        with open(error_filename, 'w') as f:
            f.write("Error: Unexpected error")
        return error_filename
